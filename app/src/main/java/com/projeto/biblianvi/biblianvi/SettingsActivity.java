...
public class SettingsActivity extends PreferenceActivity implements OnSharedPreferenceChangeListener {
   ...
   private ProgressDialog p;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        /*
        Preference buttonAtualizar =      findPreference("updateButton");
        executor = Executors.newFixedThreadPool(3);

        buttonAtualizar.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                                                         @Override
                                                         public boolean onPreferenceClick(Preference preference) {

                                                             Activity ac = SettingsActivity.this;
                                                             p = new ProgressDialog(SettingsActivity.this);

                                                             // Runnable run = icon_new UpdateRunnable(ac, icon_new Handler()).force(true);
                                                             // executor.execute(run);
                                                             // new UpdateRunnable(ac, new Handler()).force(false).start();
