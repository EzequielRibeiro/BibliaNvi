...
/**
 * It is used to verify if a icon_new update exists.
...
public class UpdateRunnable implements Runnable {
    ...
    /**
     * Each time you enter in an Activity which for example called: <b>icon_new UpdateRunnable(this, icon_new Handler()).start();</b>
     ...
     * <li>false - use it for automatic verification of icon_new updates</li>
     * </ul>
     */
    private boolean force = false;
    /**
     * Represents if a icon_new update exists or not on the Google Play Store
     */
    ...
    /**
     * If true it means that the user has explicitly asked to verify if a icon_new update exists so the runnable will show a dialog even if
            ...
            public void run() {
                if (force) {
                    ...
                    progress_dialog = ProgressDialog.show(activity,
                            context.getResources().getText(R.string._please_wait),
                            context.getResources().getText(R.string._update_test),
                            true,
                            false);

                    progress_dialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Cancelar", new DialogInterface.OnClickListener() {
                        @Override
                    ...
                    progress_dialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                        @Override
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (update_available) {
                    ...
                    saveValue.putBoolean("versaoDisponivel",true);
                    saveValue.putString("versaoAviso","Nova versão disponível no Google Play: "+ novaVersao);
                    saveValue.commit();
                    ...
                    saveValue.putBoolean("versaoDisponivel",false);
                ...
                Log.i("Versao atual",current_version);
                Log.i("Versao online",novaVersao);
            }
        });
    }
    ...
    private boolean update_available() {
        ...
        // Sets icon_new instant of time in which it has checked the update
    ...
    private boolean web_update() {
        ...
        try {
            novaVersao = Jsoup.connect("https://play.google.com/store/apps/details?id=" + package_name + "&hl=it")
                    .timeout(30000)
                    ...
                    .first()
                    .ownText();
            Log.i("Versao web_update",novaVersao);

        } catch (Exception e) {
            FirebaseCrash.report(e.getCause());
        }

        return newer_version_available(current_version, novaVersao);
    ...
    private void show_dialog_you_are_not_updated() {
        ...
        alertDialogBuilder.setTitle(context.getString(R.string._you_are_not_updated_title) + ":"+novaVersao);
        alertDialogBuilder.setMessage(context.getString(R.string._you_are_not_updated_message));
        alertDialogBuilder.setIcon(getCloudDrawable());
        alertDialogBuilder.setCancelable(false);
        alertDialogBuilder.setNegativeButton(R.string._no, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
        dialog.cancel();
                OrientationUtils.unlockOrientation(activity);
            }
        });
        alertDialogBuilder.setPositiveButton(R.string._yes, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
    ...
    private void show_dialog_you_are_updated() {
        ...
        alertDialogBuilder.setTitle(context.getString(R.string._you_are_updated_title));
        alertDialogBuilder.setMessage(context.getString(R.string._you_are_updated_message));
        alertDialogBuilder.setIcon(getCloudDrawable());
        alertDialogBuilder.setCancelable(false);
        alertDialogBuilder.setPositiveButton(R.string._ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                dialog.cancel();
                OrientationUtils.unlockOrientation(activity);
                SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(activity.getApplicationContext());
                SharedPreferences.Editor saveValue = preferences.edit();
                saveValue.putString("novaVersao", "");
                saveValue.commit();
    ...
    private static String getLastUpdateTestKey(Context context){
        return context.getString(R.string._last_update_test_preferences) + "_" + context.getPackageName();
    }
